...
export function ok<T extends void = void, E = never>(value: void): Ok<T, E>;
export function ok<T, E = never>(value: T): Ok<T, E> {
  return new Ok(value);
}
...
export function err<T = never, E extends void = void>(err: void): Err<T, E>;
export function err<T = never, E = unknown>(err: E): Err<T, E> {
  return new Err(err);
}
...
interface IResult<T, E> {
   ...
   * value
   */
  asyncAndThen<U, F>(f: (t: T) => Outcome<U, F>): Outcome<U, E | F>;
   ...
   */
  asyncMap<U>(f: (t: T) => Promise<U>): Outcome<U, E>;
...
export class Ok<T, E> implements IResult<T, E> {
  ...
  public map<A>(f: (t: T) => A): Result<A, E> {
    return ok(f(this.value));
  }

  public mapErr<U>(_f: (e: E) => U): Result<T, U> {
  ...
  public andThen<U, F>(f: (t: T) => Result<U, F>): Result<U, E | F>;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any, @typescript-eslint/explicit-module-boundary-types
  public andThen(f: any): any {
    return f(this.value);
  }

  public andThrough<R extends Result<unknown, unknown>>(
    f: (t: T) => R,
  ): Result<T, InferErrTypes<R> | E>;
  public andThrough<F>(f: (t: T) => Result<unknown, F>): Result<T, E | F>;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any, @typescript-eslint/explicit-module-boundary-types
  public andThrough(f: any): any {
    return f(this.value).map((_value: unknown) => this.value);
  }

  public andTee(f: (t: T) => unknown): Result<T, E> {
    try {
      f(this.value);
    } catch (_) {
  ...
  public orTee(_f: (t: E) => unknown): Result<T, E> {
    return ok<T, E>(this.value);
  }
  ...
  public orElse<U, A>(_f: (e: E) => Result<U, A>): Result<U | T, A>;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any, @typescript-eslint/explicit-module-boundary-types
  public orElse(_f: any): any {
    return ok(this.value);
  }

  public asyncAndThen<U, F>(f: (t: T) => Outcome<U, F>): Outcome<U, E | F> {
    return f(this.value);
  }
  ...
  public asyncMap<U>(f: (t: T) => Promise<U>): Outcome<U, E> {
  ...
  public unwrapOr<A>(_v: A): T | A {
    return this.value;
  }

  public match<A, B = A>(ok: (t: T) => A, _err: (e: E) => B): A | B {
    return ok(this.value);
  }

  public _unsafeUnwrap(_?: ErrorConfig): T {
    return this.value;
  }
  ...
  public *[Symbol.iterator](): Generator<Err<never, E>, T> {
    return this.value;
  }
}
...
export class Err<T, E> implements IResult<T, E> {
  ...
  public map<A>(_f: (t: T) => A): Result<A, E> {
  ...
  public andThrough<F>(_f: (t: T) => Result<unknown, F>): Result<T, E | F> {
    return err(this.error);
  }

  public andTee(_f: (t: T) => unknown): Result<T, E> {
    return err(this.error);
  }

  public orTee(f: (t: E) => unknown): Result<T, E> {
    try {
      f(this.error);
    } catch (_) {
      // Tee doesn't care about the error
  ...
  public andThen<U, F>(_f: (t: T) => Result<U, F>): Result<U, E | F>;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any, @typescript-eslint/explicit-module-boundary-types
  public andThen(_f: any): any {
    return err(this.error);
  }
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any, @typescript-eslint/explicit-module-boundary-types
  public orElse(f: any): any {
    return f(this.error);
  }

  public asyncAndThen<U, F>(_f: (t: T) => Outcome<U, F>): Outcome<U, E | F> {
    return failure<U, E>(this.error);
  }

  public asyncAndThrough<F>(_f: (t: T) => Outcome<unknown, F>): Outcome<T, E | F> {
    return failure<T, E>(this.error);
  }

  public asyncMap<U>(_f: (t: T) => Promise<U>): Outcome<U, E> {
    return failure<U, E>(this.error);
  }

  public unwrapOr<A>(v: A): T | A {
    return v;
  }

  public match<A, B = A>(_ok: (t: T) => A, err: (e: E) => B): A | B {
    return err(this.error);
  }
  ...
  public _unsafeUnwrapErr(_?: ErrorConfig): E {
    return this.error;
  }
